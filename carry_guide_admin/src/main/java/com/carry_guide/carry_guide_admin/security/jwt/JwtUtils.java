...
public class JwtUtils {
    ...
    public String generateTokenFromUsername(UserAccountDetails userAccountDetails) {
        String username = userAccountDetails.getUsername();
        String userType = userAccountDetails.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .collect(Collectors.joining(","));
        return Jwts.builder()
                .subject(username)
                .claim("user_type", userType)
                .claim("is2faEnabled", userAccountDetails.isIs2faEnabled())
                .issuedAt(new Date())
